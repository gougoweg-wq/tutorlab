...
type Rule = { key: number; topicId: string; count: number; dmin: number; dmax: number; inBank: number | null };
...
export function NewTest({ topics, students }: { topics: Topic[]; students: { id: string; name: string; grade: number }[] }) {
  ...
  const [rules, setRules] = React.useState<Rule[]>([{ key: 1, topicId: topics[0]?.id ?? "", count: 5, dmin: 2, dmax: 4, inBank: null }]);
  ...
  React.useEffect(() => {
    rules.filter((r) => r.inBank === null && r.topicId).forEach(async (r) => {
      const res = await countForRuleAction(r.topicId, r.dmin, r.dmax);
      setRules((rs) => rs.map((x) => (x.key === r.key ? { ...x, inBank: res.ok ? res.data : 0 } : x)));
    });
  }, [rules]);
  const patch = (key: number, p: Partial<Rule>) => setRules((rs) => rs.map((r) => (r.key === key ? { ...r, ...p, inBank: "topicId" in p || "dmin" in p || "dmax" in p ? null : r.inBank } : r)));
        ...
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget);
          start(async () => {
            const res = await createTestAction({ title: fd.get("title"), dueAt: fd.get("due") ? new Date(String(fd.get("due"))).toISOString() : null, maxAttempts: fd.get("attempts"), timeLimitMin: fd.get("limit") || null, studentIds: [...picked],
              rules: rules.map((r) => ({ topicId: r.topicId, includeSubtree: true, count: r.count, difficultyMin: r.dmin, difficultyMax: r.dmax, types: null })) });
          ...
          <div>
            ...
            <div className="space-y-2">
              {rules.map((r) => (
                <div key={r.key} className="grid grid-cols-[1fr_auto] sm:grid-cols-[1fr_84px_150px_auto] gap-2 items-center">
            ...
            <div className="mt-3 flex items-center justify-between"><Button type="button" variant="ghost" size="sm" onClick={() => setRules((rs) => [...rs, { key: Date.now(), topicId: topics[0]?.id ?? "", count: 5, dmin: 2, dmax: 4, inBank: null }])}><Plus />{t("addTopic")}</Button><span className="t-small text-muted tnum">{t("total", { n: total })}</span></div>
          </div>
